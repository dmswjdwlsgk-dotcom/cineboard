...
⚠️ Many scripts (especially documentary/biography narration) mix concrete action sentences with the narrator's ABSTRACT COMMENTARY — rhetorical questions, summary judgments, transitions ("이건 협박이 아니었습니다", "우리는 질문해야 합니다", "그런데 그 계산이 틀렸습니다"), meta remarks about how people generally VIEW or DEBATE the subject ("사람들은 보통 두 갈래로 갈립니다", "혹자는 폭군이라, 혹자는 성군이라 부릅니다"), and CHANNEL META-ADDRESS (subscribe/like requests, "다음 이야기에서 뵙겠습니다", "댓글로 남겨주세요") — all of these describe NO physical historical action at all.
⚠️ NARRATOR/PRESENTER IS INVISIBLE, NO EXCEPTIONS: for ALL of the above, do NOT render a modern lecturer, professor, presenter, documentary host, lecture hall, classroom of debating scholars, projector screen, video production studio, or any other MODERN setting/human — even though the segment has no concrete historical action, it is still set in the world of this video's era and visual style, never in a modern real-world meta-space. If tempted to draw "people discussing/debating the subject," that reads as a modern panel/lecture — avoid it entirely.
⚠️ Instead, default to a QUIET CALLBACK SHOT: reuse an already-established location/object from this story, or the main historical figure themselves (in the established art style), shown in a still, symbolic wide shot or silhouette — like a documentary camera lingering on its subject, not cutting to a host or classroom.
