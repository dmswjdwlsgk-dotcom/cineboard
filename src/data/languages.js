...
const FOREIGN_SETTING_GROUPS = [
  ...
  /페루|안데스|리마|브라질|아르헨티나|칠레|콜롬비아|멕시코|쿠바|볼리비아|에콰도르|베네수엘라|남미|라틴아메리카/,
  /북유럽|바이킹|스칸디나비아|노르드/,
]
...
export function isNativeSetting(lang, scriptText) {
  ...
  if (foreign > 0) return false                         // 외국 무대 신호가 있으면 외국
  // 한국 신호가 문턱 아래지만 0은 아니면(짧은 야담의 양반·사또 두어 번) 한국으로 본다.
  // 한국 신호가 아예 0이면 한국이 아니다 — 조선이든 현대 한국이든 대본에 한국 낱말이
  // 하나도 안 나올 수는 없다. 외국 목록에 없는 나라(페루 융가이 1970)가 이 길로 들어와
  // 한국으로 판정돼 갓·두루마기가 나왔다. 나라 이름을 목록에 계속 보태는 대신 기본값을
  // 뒤집는다: 신호 없음 → 대본에서 유추(derived).
  return (dynasty + weak + modern) > 0
}
